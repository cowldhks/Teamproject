	...
	*/
	...
	while (fin.read((char*)&c, sizeof(c)))		//파일의 끝까지 모든 레코드를 읽겠다
	{
		if (strcmp(NAME, c.name) == 0)		//파일의 내용과 같으면 정보출력
		{
			...
			gotoxy(40, 20);cout << "car_no : " << c.Car_No;
			break;
		}
	}
	fin.close();
...
void ROOMS::status()																//방 상태를 보여주는 함수
{
	...
	fstream fin("rooms", ios::binary | ios::in);
	gotoxy(5, 19);cout << "  R.No    Room Type  Room Charges   Grade      Status" << "\n\n";
	...
	for (int i = 1;i<200;i++)															//방이 200개 이므로 int i=1부터 200
	{
		...
		r.grade = ggrade[i]; cout << r.grade << "\t";									//방의 별점을 화면에 출력해준다.											
...
void CUSTOMER::check_in()															//객실 예약에 대한 함수
{
	...
	gotoxy(28, 3); cout << "H I L L  V I E W";
	gotoxy(28, 5);cout << "C H E C K    I N";
	gotoxy(10, 10);cout << "Enter the date (DD-MM-YY) : ";
	getdate(&d);
	DAY = d.da_day;
	MONTH = d.da_mon;
	...
	else                                                                           //방을 찾을 수 있고 예약할 수있다
	{
		...
		cin >> PHONE;
		gotoxy(5, 19); cout << "Enter th Car No. : "; cin >> carno;
		gotoxy(5, 20); cout << "Enter the people No. : "; cin >> peopleno; //이름, 주소, 번호, 차번호, 사람의수를 입력받는다
		gotoxy(5, 21); cout << "Enter number of meal. : "; cin >> meal;

		c.Roomn = roomno;
		c.daysStayed = -1;
		c.amount = -1;
		c.Car_No = carno;
		c.People_no = peopleno;
		c.meal = meal;

		strcpy(c.name, NAME);
		strcpy(c.add, ADD);
		strcpy(c.phone, PHONE);													//입력받은 값들을 고객 파일에 넣어준다.
		c.DAY = DAY;
		c.MONTH = MONTH;
		c.YEAR = YEAR;
...
void CUSTOMER::check_out()
{
	...
	while (fin1.read((char*)&c, sizeof(c)))
	{
		if (roomno == c.Roomn && c.daysStayed == -1)   //룸넘버와 파일에서 읽어들인 (고객의)룸넘버와 같고 호텔 투박수가 -1일때 (-1일때 지정되지 않음을 뜻한다)
		{
			...
			if (c.People_no < 3)	c.amount = D*AMOUNT + meal*10000;		//총지불금액 = 머문날*객실금액*(인원수/2)+밥값 / 3명이상일때는 추가인원비례 3명미만일때는 정상가격
			else c.amount = (D*AMOUNT + meal*10000)*(c.People_no / 2);
			...
			system("cls");
			if (ballett == 1) {
				gotoxy(18, 8); cout << "Car_no [" << c.Car_No << "]  is Complete application for a valet";
				gotoxy(18, 12); cout << "Did you satisfied? give theam a grade (1~10) : "; cin >> ggrade[roomno - 1];
				gotoxy(26, 14); cout << "Thx :) Good bye!";
			}
			else if (ballett == 2) {
				gotoxy(18, 11);	 cout << "Did you satisfied? give theam a grade (1~10) : "; cin >> ggrade[roomno - 1];
				gotoxy(26, 14); cout << "Thx :) Good bye!";
			}
...
void enter_password()				//처음메인화면 들어갈때 비밀번호 함수
{
	...
	char pass[10],c;	//pass: 비밀번호 버퍼, c: 입력문자의 아스키코드
	int i =0;
	while(true)
	{
	...
	{
	gotoxy(28,20);cout <<"   WRONG PASSWORD    " ;		// 잘못된 비밀번호 입력
	gotoxy(50,18);
	for(int j = 0; j=i;j++)
	{
		pass[i] = '\0';cout<<'\b'<<' '<<'\b';		//입력된 비밀번호 지움
		i--;
	}
	continue;		//비밀 번호 다시 치기 위한 반복문 다시수행
