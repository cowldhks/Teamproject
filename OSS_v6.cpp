...
void CUSTOMER::check_in()															//객실 예약에 대한 함수
{
	...
	else                                                                           //방을 찾을 수 있고 예약할 수있다
	{
		...
		gets_s(PHONE);
		gotoxy(5, 19); cout << "Enter the Car No. : "; cin >> Car_No;

		gotoxy(5, 20); cout << "Enter the people No. : "; cin >> People_no; //(채)
		...
		strcpy_s(c.add, ADD);
		strcpy_s(c.phone, PHONE);													//이름, 주소, 번호, 차번호를 입력받는다
...
{
	//수정코드
	_getch();																	//3을 입력한뒤 화면이 바로 넘어가는것을 방지하기 위해							
	system("cls");
	basic();
	int roomno, flag = 0;
	long double AMOUNT;
	...
	while (fin1.read((char*)&c, sizeof(c)))
	{
		if (roomno == c.Roomn && c.daysStayed == -1)
		{
			...
			system("cls");
			if (ballett == 1) {
				gotoxy(18, 10); cout << "차 번호 " << Car_No << "발렛 서비스 신청 완료. 대기중입니다.";
