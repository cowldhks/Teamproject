   ...
   while(fin1.read((char*)&c, sizeof(c)))
   {
      if(roomno==c.Roomn && c.daysStayed==-1)   //룸넘버와 파일에서 읽어들인 (고객의)룸넘버와 같고 호텔 투박수가 -1일때 (-1일때 지정되지 않음을 뜻한다)
      {
         ...
         c.daysStayed=D; 
    	  ...
    	  if(c.People_no < 3)	c.amount = D*AMOUNT + meal;		//총지불금액 = 머문날*객실금액*(인원수/2)+밥값 / 3명이상일때는 추가인원비례 3명미만일때는 정상가격
		  else c.amount = (D*AMOUNT + meal)*(c.People_no/2);
			...
			system("cls");
			if(ballett == 1) {gotoxy(18, 8); cout<<"Car_no ["<<c.Car_No<<"]  is Complete application for a valet";
							  gotoxy(18, 12); cout<<"Did you satisfied? give theam a grade (1~10) : "; cin>>ggrade[roomno-1];
							  gotoxy(26, 14); cout<<"Thx :) Good bye!" ;}
			else if(ballett == 2) { gotoxy(18, 11);	 cout << "Did you satisfied? give theam a grade (1~10) : "; cin>>ggrade[roomno-1];
						gotoxy(26, 14); cout << "Thx :) Good bye!"; 
			}
