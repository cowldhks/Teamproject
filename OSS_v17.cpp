...
void CUSTOMER::check_in()															//객실 예약에 대한 함수
{
	...
	else                                                                           //방을 찾을 수 있고 예약할 수있다
	{
		...
		gotoxy(5, 20); cout << "Enter the people No. : "; cin >> People_no; 
   ...
   while(fin1.read((char*)&c, sizeof(c)))
   {
      if(roomno==c.Roomn && c.daysStayed==-1)   //룸넘버와 파일에서 읽어들인 (고객의)룸넘버와 같고 호텔 투박수가 -1일때 (-1일때 지정되지 않음을 뜻한다)
      {
         ...
         c.daysStayed=D; 
    	  ...
    	  if(People_no < 3)	c.amount = D*AMOUNT + meal;		//총지불금액 = 머문날*객실금액*(인원수/2)+밥값 / 3명이상일때는 추가인원비례 3명미만일때는 정상가격
		else c.amount = (D*AMOUNT + meal)*(People_no/2);
			...
			else if(ballett == 2) { gotoxy(26, 11);	 cout << "만족하셨나요? 별점을 주세요 (1~10점) : "; cin >> r.grade;
						gotoxy(26, 14); cout << "감사합니다 :) 안녕히가세요!"; 
			}
